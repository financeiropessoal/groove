...
const EditMusicianRepertoirePage: React.FC = () => {
    const { musician, updateMusicianProfile } = useMusicianAuth();
    const { showToast } = useToast();
    
    const [repertoire, setRepertoire] = useState<Song[]>(musician?.repertoire || []);
    ...
    const handleSave = async () => {
        setIsSaving(true);
        try {
            await updateMusicianProfile({ repertoire });
        ...
        <div>
            <div className="flex flex-col md:flex-row justify-between md:items-center mb-8 gap-4">
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        <h1 className="text-3xl font-bold">Gerenciar Repertório</h1>
                        <p className="text-gray-400">Liste as músicas que você sabe tocar.</p>
                    </div>
                </div>
...
export default EditMusicianRepertoirePage;