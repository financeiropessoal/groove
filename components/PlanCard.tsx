...
const PlanCard: React.FC<PlanCardProps> = ({ plan, isSelected, onSelect, isVenueAuthenticated }) => {
    return (
        <div 
            className={`bg-gray-900 rounded-lg overflow-hidden shadow-lg border-2 transition-all duration-300 cursor-pointer ${isSelected ? 'border-red-500 scale-102' : 'border-gray-700 hover:border-red-600'}`}
            onClick={onSelect}
        >
            <div className="p-6">
                <div className="flex flex-col sm:flex-row justify-between sm:items-start">
                    ...
                    <div className="text-left sm:text-right mt-4 sm:mt-0 sm:ml-4 flex-shrink-0">
                        {isVenueAuthenticated ? (
                            <p className="text-2xl font-bold text-red-500">R$ {plan.price.toFixed(2).replace('.',',')}</p>
                        ) : (
            ...
            {isSelected && (
                <div className="bg-gray-800 p-6 border-t-2 border-red-500 animate-fade-in">
